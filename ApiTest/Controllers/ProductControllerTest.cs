    ...
    public class ProductControllerTest
    {
        ...
        public void Setup()
        {
            ...
            _products = new List<ProductModel> {
                     ...
                     new ProductModel{ Id=1,Name="T-Shirt",Description="Rugger T-Shirt",Price=600 },
                     new ProductModel{ Id=1,Name="Jean",Description="Levis Jean",Price=1200 },
                     new ProductModel{ Id=1,Name="Cap",Description="Sports Cap",Price=300 },
           };

            Mock<IProductProvider> provider = new Mock<IProductProvider>();
            provider.Setup(k => k.GetProducts()).ReturnsAsync(_products);
            provider.Setup(k => k.LoadProduct(It.IsAny<int>())).ReturnsAsync(_products[0]);
            provider.Setup(k => k.SaveProduct(It.IsAny<ProductModel>())).Returns(Task.FromResult(false));
        ...
        {
            GetProductsResponse response = await _controller.GetProducts();
